...
if ($result->num_rows > 0) {
    ...
    // Now $product['product_image'] has your image data (if stored as BLOB)
}
?>
<?php
include('db.php');
...
if (isset($_GET['id'])) {
    ...
    if ($product = $product_result->fetch_assoc()) {
        $category_id = $product['category_id']; // Correct column name

        // Related products
        $related_stmt = $conn->prepare("SELECT * FROM product WHERE category_id = ? AND id != ? LIMIT 6");
...
<!-- Mirrored from caketheme.com/html/mojuri/shop-details.html by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 16 May 2025 05:09:47 GMT -->
<head>
		<!-- Meta Data -->
		<meta charset="UTF-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<title>Shop Details | Royal-carat || Royal Gold & Diamond</title>
							...
							<div class="section-container large p-l-r">
								<div class="row">
									<div class="col-xl-3 col-lg-3 col-md-12 col-sm-12 col-12 header-left">
										<div class="site-logo">
											<a href="index.php">
												<img width="400" height="140" src="media/royal-carat-logo-dark.png" alt="Royal-carat || Royal Gold & Diamond" />
											</a>
										</div>
									</div>
						...
						<div id="title" class="page-title">
							<div class="section-container">
								...
								<div class="breadcrumbs">
									<a href="index.php">Home</a><span class="delimiter"></span><a href="all-Jewellery.php">Shop</a><span class="delimiter"></span><?php echo htmlspecialchars($product['product_description']); ?>
								</div>
							</div>
						</div>
							...
							<div class="shop-details zoom" data-product_layout_thumb="scroll" data-zoom_scroll="true" data-zoom_contain_lens="true" data-zoomtype="inner" data-lenssize="200" data-lensshape="square" data-lensborder="" data-bordersize="2" data-bordercolour="#f9b61e" data-popup="false">	
								<div class="product-top-info">
									<div class="section-padding">
                						...
                						<?php
// Always use image as file path only
$productImage = 'royal-carat-admin/' . $product['product_image'];
?>

<img id="productImageMain" width="900" height="900" src="<?php echo htmlspecialchars($productImage); ?>" alt="" title="">
												...
												<div class="product-info col-lg-5 col-md-12 col-12 ">
													<h1 id="productInfoMain" class="title" ><?php echo htmlspecialchars($product['product_description']); ?></h1>
													<!-- <span class="price">
													...
													<div class="btn-add-to-cart">
																<button class="add-to-cart-button" onclick="openModal()">Inquiry Now</button>
															</div>
														<!-- </div> -->
														 <div id="cartModal" class="modal" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.5); z-index:999;">
  <div class="modal-content" style="background:#fff; padding:20px; max-width:500px; margin:100px auto; text-align:center; position:relative; border-radius:8px;">
    
    <span class="close" onclick="closeModal()" style="position:absolute; top:10px; right:10px; cursor:pointer; font-size:24px;">&times;</span>

    <h2 id="modalProductInfo" style="margin-bottom:20px;"></h2>
    <img id="modalProductImage" src="" width="200" height="200" alt="Product Image" style="margin-bottom:20px;">

    <form id="cartForm" method="post" action="send_mail.php" style="text-align:left;">
      <input type="hidden" name="product_info" id="formProductInfo">
      <input type="hidden" name="product_image" id="formProductImage">

      <label for="name">Name:</label><br>
      <input type="text" id="name" name="name" required style="width:100%; padding:8px; margin-bottom:10px;"><br>

      <label for="contact">Contact Number:</label><br>
      <input type="text" id="contact" name="contact" required style="width:100%; padding:8px; margin-bottom:10px;"><br>

      <label for="email">Email:</label><br>
      <input type="email" id="email" name="email" required style="width:100%; padding:8px; margin-bottom:20px;"><br>

      <button type="submit" style="padding:10px 20px; background:#4CAF50; color:#fff; border:none; border-radius:4px; cursor:pointer;">Submit</button>
    </form>
  </div>
</div>

														<!-- <div id="cartModal" class="modal" style="display:none; position: fixed; top:0; left:0; right:0; bottom:0; background: rgba(0,0,0,0.5); z-index:999;">
  <div class="modal-content" style="background: #fff; padding: 20px; max-width: 500px; margin: 100px auto; text-align:center; position: relative;">
    ...
    <img id="modalProductImage" src="" width="200" height="200" alt="Product Image">

    <form id="cartForm" method="post" action="send_mail.php">
      <input type="hidden" name="product_info" id="formProductInfo">
      <input type="hidden" name="product_image" id="formProductImage">
      ...
      <button type="submit">Submit</button>
													...
													<div class="product-meta">
														...
														<span class="posted-in">Category: <a href="shop-grid-left.html" rel="tag"><?php echo htmlspecialchars($product['product_description']); ?></a></span>
    ...
    $mainImage = 'royal-carat-admin/' . $row['product_image'];
    $hoverImage = !empty($row['hover_image']) ? 'royal-carat-admin/' . $row['hover_image'] : $mainImage;
?>
                ...
                <div class="products-thumb">
                    <div class="product-thumb-hover">
                        <a href="shop-details.php?id=<?php echo $row['id']; ?>">
                            <img width="600" height="600" src="<?php echo $mainImage; ?>" class="post-image" alt="">
                            <img width="600" height="600" src="<?php echo $hoverImage; ?>" class="hover-image back" alt="">
                        </a>
                    </div>		
                </div>
...
if (isset($_GET['success'])) {
    if ($_GET['success'] == 1) {
        ...
        echo "<script>alert('Failed to send inquiry. Please try again later.');</script>";
    }
}
?>
...
<script>
  function openModal() {
    // Get product details
    var productInfo = document.getElementById('productInfoMain').innerText;
    var productImageSrc = document.getElementById('productImageMain').src;

    // Set modal values
    document.getElementById('modalProductInfo').innerText = productInfo;
    document.getElementById('modalProductImage').src = productImageSrc;

    // Set hidden form values
    document.getElementById('formProductInfo').value = productInfo;
    document.getElementById('formProductImage').value = productImageSrc;
